...
def get_data_path(root='examples'):
    ...
    lm_path = [os.path.join(i.replace(i.split(os.path.sep)[-1],''),'landmarks',i.split(os.path.sep)[-1]) for i in lm_path]
    mask_path = [os.path.join(i.replace(i.split(os.path.sep)[-1],''),'mask',i.split(os.path.sep)[-1]) for i in im_path]

    return im_path, lm_path, mask_path
...
def main(rank, opt, name='examples'):
    ...
    im_path, lm_path, mask_path = get_data_path(name)
    ...
    for i in range(len(im_path)):
        ...
        if not os.path.isfile(lm_path[i]):
            ...
            continue
        im_tensor, lm_tensor, mask_tensor = read_data(im_path[i], lm_path[i], mask_path[i], lm3d_std)
        data = {
            'imgs': im_tensor,
            'lms': lm_tensor,
            'msks': mask_tensor 
