...
class FaceReconModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            ...
            parser.add_argument('--w_penalty', type=float, default=0, help='weight for penalty')
    ...
    def __init__(self, opt):
        ...
        meanshape = self.facemodel.mean_shape.reshape(-1, 3)
        self.translate = torch.tensor([(meanshape[:, 0].max()+meanshape[:, 0].min())/2, (meanshape[:, 1].max()+meanshape[:, 1].min())/2, (meanshape[:, 2].max()+meanshape[:, 2].min())/2]).unsqueeze(0)
    ...
    def save_uv(self, tex_name, shape_name):
        np.save(shape_name, self.uv_shape)

        np.save(tex_name, (np.clip(self.uv_tex, 0, 1) * 255).astype(np.uint8))
