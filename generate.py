...
def generate_images(
    ...
    for fname in tqdm(fnames):
        ...
        img = G(z=z, cond_im=cond_im, c=label, truncation_psi=truncation_psi, noise_mode=noise_mode)

        #img = (img - (-1.5)) * (255 / (1.5 - (-1.5)))
        #img = torch.round(img.permute(0, 2, 3, 1)).clamp(0, 255).to(torch.uint8)
        img = img.permute(0, 2, 3, 1).squeeze()
        np.save(f'{outdir}/{os.path.basename(fname)}', img.cpu().numpy())
        #save_image(img, f'{outdir}/{os.path.basename(fname.split(".")[-2])}.png')
        #PIL.Image.fromarray(img[0].cpu().numpy(), 'RGB').save(f'{outdir}/{os.path.basename(fname.split(".")[-2])}.png')
