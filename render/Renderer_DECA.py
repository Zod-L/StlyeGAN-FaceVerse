...
class SRenderY(nn.Module):
    ...
    def forward(self, vertices, transformed_vertices, albedos, lights=None, h=None, w=None, light_type='point', background=None):
        ...
        ####
        # vis mask
        alpha_images = rendering[:, -1, :, :].unsqueeze(1).repeat((1,3,1,1))#[:, None, :, :].detach()
        ...
        if lights is None:
            ...
            '''
            lights = torch.ones(batch_size, 2, 6).to(normal_images.device)
            lights[:,1,0] = -1
            lights[:,:,3:] *= 1.5          
            light_type='dir'
            '''
        ...
        if lights is not None:
            if lights.shape[1] == 9:
                ...
                if light_type=='point':
                    ...
                    #print('\n=== shaidng: ', shading.shape)
                    shading_images = shading.reshape([batch_size, albedo_images.shape[2], albedo_images.shape[3], 3]).permute(0,3,1,2)
            #images = albedo_images*shading_images
            images = (albedo_images*0.5+0.5)*(shading_images*0.5+0.5)
            images = images * 2 -1
        else:
    ...
    def add_directionlight(self, normals, lights):
        ...
        #print('\n=== normals_dot_lights:', normals_dot_lights.max(),normals_dot_lights.min())
        #normals_dot_lights = normals_dot_lights * 0.5 + 0.5
        #normals_dot_lights = torch.clamp(normals_dot_lights, 0., 1.)
        shading = normals_dot_lights[:,:,:,None]*light_intensities[:,:,None,:]
        return shading.mean(1)
